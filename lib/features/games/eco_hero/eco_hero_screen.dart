...
import 'eco_hero_game.dart';

class EcoHeroGameScreen extends StatefulWidget {
  const EcoHeroGameScreen({super.key});

  @override
  State<EcoHeroGameScreen> createState() => _EcoHeroGameScreenState();
}

class _EcoHeroGameScreenState extends State<EcoHeroGameScreen> {
  int _score = 0;
  int _highScore = 0;
  int _timeLeft = 60;
  late EcoHeroGame _game;
  ...
  Future<void> _loadHighScore() async {
    ...
    setState(() {
      _highScore = prefs.getInt('eco_hero_highscore') ?? 0;
    });
  }
  ...
  Future<void> _saveHighScore(int score) async {
    final prefs = await SharedPreferences.getInstance();
    if (score > _highScore) {
      await prefs.setInt('eco_hero_highscore', score);
      setState(() {
        _highScore = score;
      });
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(AppLocalizations.of(context)!.ecoHeroTitle)),
      body: Stack(
        children: [
